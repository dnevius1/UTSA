...
//File Name:            DanielNeviusLab2.c
//Course/Section:       CS 2124 Section 0DA
//Date:                 10/9/2021
//Instructor:           Dr. Ku

/*This program creates random data in file TestData.txt to utilize in finding the time complexities of insertion sort and quick sort functions, as well as three mystery functions*/
...
functionRuntimes timeAlgorithm( char *name, int numTestCases, int numTestCaseSizes, int testCaseSizes[], void (*f)(FILE *) )
{
    ...
    //Allocating space for each array in the fRT struct and populating each variable with its respective passed value
    fRT.name = (char*)malloc(75 * sizeof(char));
    strcpy(fRT.name, name);
    fRT.numTestCases = numTestCases;
    fRT.numTestCaseSizes = numTestCaseSizes;
    fRT.testCaseSizes = (int*)malloc(numTestCases * sizeof(int));
    ...
    //Allocating space for the 2D array fRT.times that will be numTestCaseSizes long
    fRT.times = (double**)malloc(sizeof(double*) * numTestCaseSizes) /* instead malloc an array of double*s of size numTestCaseSizes */;
    for( i=0; i<numTestCaseSizes; i++ )
    {
        //Allocating space for each element of fRT.times. Each element will be numTestCases long
    ...
    /*Allocating space for the fRT.avg array of doubles and calling computeAvg() to calculate the average time
    it takes to run each function numTestCases times. There are numTestCasesSizes different inputs to test each 
    function's time*/
...
void computeAvg( functionRuntimes fRT )
{
    ...
    double avgTime = 0;
    double sum = 0;
    /*Iterating through the resulting function times stored in fRT.times 2D array, adding each runtime in 
    each test series, then dividing by fRT.numTestCases to compute the average runtime*/
    for (i = 0; i < fRT.numTestCaseSizes; i++) {
        sum = 0;
        for (j = 0; j < fRT.numTestCases; j++) {
            sum += fRT.times[i][j];
        }
        avgTime = sum / fRT.numTestCases;
        fRT.avg[i] = avgTime;
    }
...
{
    int i,j;
    //Labeling test sizes and each test number for corresponding data in the table
    printf("%s\nTest size\t", fRT.name);
    for (i = 0; i < fRT.numTestCases; i++) {
        printf("Test #%d ", i);
    }
    //Printing the time test results that correspond with each test
    printf("Average");
    printf("\n");
    for (i = 0; i < fRT.numTestCaseSizes; i++) {
...
void freeFunctionRuntimes( functionRuntimes fRT )
{
    /*Freeing the memory allocated to the arrays inside of fRT struct so that they can be re-used in the next 
    function calls*/
    free(fRT.name);
    free(fRT.times);
    free(fRT.avg);
