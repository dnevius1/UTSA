...
$sql="Select `auto_id`,`name` from `manufacturer` where `name`='Samsung'";
$result=$dblink->query($sql) or
	die("Something went wrong with: $sql<br>\n".$dblink->error);
$count=0;
while ($item=$result->fetch_array(MYSQLI_ASSOC))
{
	$sql="Select `auto_id`,`manufacturer` from `equipment_testing` where `manufacturer` = '$item[name]'";
	$rst=$dblink->query($sql) or
		die("Something went wrong with: $sql<br>".$dblink->error);
	while ($data=$rst->fetch_array(MYSQLI_ASSOC))
	{
	
	$sql="Update `equipment_testing` set `manufacturer`='$item[auto_id]' where `auto_id`='$data[auto_id]'";
	$dblink->query($sql) or
		die("Something went wrong with: $sql<br>".dblink->error);
	$count++;
	}
